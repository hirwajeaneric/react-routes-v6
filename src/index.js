...
function CourseId() {
  ...
  return (
    <div>
      ...
      <button 
      onClick={()=>{
        navigate("/dashboard", {state: {name: courseid, price: 399}})
      }}
      className='btn btn-warning'>Price</button>
      <Link to="/dashboard" state={{name: "Django", price: 300}} >Test link</Link>
    </div>
  )
}
...
function Dashboard() {
  ...
  return (
    <div>
      <h1>Info that I got here is {location.state.name}</h1>
      <p>The price is {location.state.price}</p>
    </div>
  )
}
